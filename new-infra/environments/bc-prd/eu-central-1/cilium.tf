#--------------------------------------------------------------
# Cilium CNI — aws-cni chaining mode (bc-prd)
#
# vpc-cni continues to own IPAM: pods keep real VPC IPs from the
# 10.30.10-12.0/24 private subnets. Cilium chains after vpc-cni
...
#
# bc-prd has ZERO internet egress — ALL images MUST come through the
# ECR quay/ pull-through cache. Pre-seed required before first apply
# (see plan file). data.aws_caller_identity.current is declared in
# eks-addons.tf.
#
# operator.replicas=1 because workload node group starts at desired_size=1.
# Bump to 2 if node group scales to 2+.
#
...
resource "helm_release" "cilium" {
  ...
  set {
    ...
    value = "1"   # workload node group starts at desired_size=1
  }
  set {
  ...
  # ---- Images: ECR pull-through MANDATORY (no internet egress) ----
  # Pre-seed: docker pull quay.io/cilium/<img> && docker push <ecr>/quay/cilium/<img>
  # from a machine with internet access (e.g. bc-ctrl bastion via NAT).
